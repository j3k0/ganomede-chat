...
describe('RoomManager', function() {
  ...
  const roomExpected = new RoomManager.Room(roomInfo);
  const message = {
    timestamp: Date.now(),
    from: 'alice',
    type: 'text',
  ...
  return describe('Room', function() {
    ...
    describe('.id()', function() {
      it('room id is prefixed with type', () => expect(RoomManager.Room.id(roomInfo).indexOf(service)).to.be(0));

      return it('room id ends with sorted list of user', function() {
        const users = ['x', 'a', '01', 'z'];
        const actual = RoomManager.Room.id({
          type: service,
          users
        });
    ...
    describe('#hasUser()', function() {
      it('returns true if username is participant', function() {
        ...
        return expect(room.hasUser('bob')).to.be(true);
      });

      return it('returns false otherwise', function() {
        expect(room.hasUser('joe')).to.be(false);
        return expect(room.hasUser({})).to.be(false);
      });
    });
    ...
    describe('#addMessage()', () => it('adds a message to a room', done => room.addMessage(message, function(err) {
      ...
      return redisClient.lrange(manager.messagesKey(roomId), 0, -1,
    ...
    return describe('#messages()', () => it('retrieves Room\'s messages for given room', done => room.messages(function(err, messages) {
      expect(err).to.be(null);
